...
#ifndef SRC_NewLookupTable_H_
#define SRC_NewLookupTable_H_
#include "tensor.h"
#include "Utiltensor.h"
#include "LookupTable.h"
// #include "NewUtiltensor.h"
#include "MyLib.h"
...
public:
  ...
  // }
    ...
    _ftE = NewTensor<xpu>(Shape2(_nVSize, _nDim), d_one);
    for (int idx = 0; idx < _nVSize; idx++) {
      if (idx < originTable._nVSize) {
        for (int idy = 0; idy < _nDim; idy++) {
          _E[idx][idy] = originTable._E[idx][idy];
        }
      } else {
        for (int idy = 0; idy < _nDim; idy++) {
          _E[idx][idy] = originTable._E[0][idy];
        }
      }
    }
    _bFineTune = true;
