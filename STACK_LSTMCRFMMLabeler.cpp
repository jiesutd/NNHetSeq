...
void Labeler::train(const string& trainFile, const string& devFile, const string& testFile, const string& modelFile, const string& optionFile,
  ...
  // extend base model alphabet
  int old_base_word_num = m_base_wordAlphabet.size();
  cout << "Original  base word alphabet size:"<< old_base_word_num <<endl;
  for (int idx = 0; idx < m_wordAlphabet.size(); ++idx) {
    m_base_wordAlphabet.set_fixed_flag(true);
    std::string word = m_wordAlphabet.from_id(idx);
  ...
  cout << "Extended base word alphabet size:"<< m_base_wordAlphabet.size()<< ", extend num:"<< extend_word_num <<endl;
  
  
  m_classifier.init(extend_word_num, wordEmb, m_options.wordcontext, charEmb, m_options.charcontext, tagEmbs, m_labelAlphabet.size(), m_options.charhiddenSize, m_options.rnnHiddenSize, m_options.hiddenSize);
...
void Labeler::loadBaseModelFile(const string& inputModelFile) {
  ...
  std::cout << "Origin base model word alphabet size: " << m_base_wordAlphabet.size() << std::endl;
}
...
void Labeler::writeModelFile(const string & outputModelFile) {
  ...
  m_base_wordAlphabet.writeModel(outf);
  m_base_charAlphabet.writeModel(outf);
  m_base_labelAlphabet.writeModel(outf);
  m_base_featAlphabet.writeModel(outf);
